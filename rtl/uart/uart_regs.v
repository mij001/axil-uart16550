...
module uart_regs #(
    ...
    output reg               rxf_push,
    output reg  [10:0]       rxf_din,
    output reg               rxf_pop,
    output reg               rxf_clear,
    input  wire [10:0]       rxf_dout,
    input  wire [4:0]        rxf_count,

    // transmit FIFO and transmitter
    output reg               txf_push,
    output reg  [7:0]        txf_din,
    output reg               txf_pop,
    output reg               txf_clear,
    input  wire [4:0]        txf_count,
    input  wire              tx_pop,          // the transmitter takes a character
    input  wire              tx_busy,
    ...
    output reg  [6:0]        lcr,             // LCR 6:0; DLAB stays inside
    output reg  [4:0]        mcr,
    input  wire [3:0]        msr_now,         // {DCD, RI, DSR, CTS}
    ...
    reg [7:0] dll_q,       dll_d;
    reg [7:0] dlm_q,       dlm_d;
    reg       fen_q,       fen_d;        // FCR0: FIFO mode
    reg [1:0] trig_q,      trig_d;       // FCR7:6

    reg       oe_q,        oe_d;         // LSR1
    reg       pe_q,        pe_d;         // LSR2
    reg       fe_q,        fe_d;         // LSR3
    reg       bi_q,        bi_d;         // LSR4
    reg       lsr7_q,      lsr7_d;       // sticky part of LSR7
    reg [4:0] errs_q,      errs_d;       // characters with errors in the RX FIFO
    reg       top_new_q,   top_new_d;    // a character became the RX head at the last edge

    reg       thri_q,      thri_d;       // THRE interrupt pending
    reg       thre_prev_q, thre_prev_d;  // THRE one cycle ago
    reg [3:0] delta_q,     delta_d;      // MSR3:0 {DDCD, TERI, DDSR, DCTS}
    reg [3:0] msr_prev_q,  msr_prev_d;   // msr_now one cycle ago
    reg [9:0] to_cnt_q,    to_cnt_d;     // character timeout counter, in ticks
    reg       to_pend_q,   to_pend_d;    // character timeout pending
    reg       irq_q,       irq_d;
    ...
    wire wr_ok      = reg_wr & reg_wstrb0;
    wire thr_write  = wr_ok & (reg_waddr == A_DATA) & ~dlab;
    wire dll_write  = wr_ok & (reg_waddr == A_DATA) &  dlab;
    wire ier_write  = wr_ok & (reg_waddr == A_IER)  & ~dlab;
    wire dlm_write  = wr_ok & (reg_waddr == A_IER)  &  dlab;
    wire fcr_write  = wr_ok & (reg_waddr == A_IIR);
    wire lcr_write  = wr_ok & (reg_waddr == A_LCR);
    wire mcr_write  = wr_ok & (reg_waddr == A_MCR);
    wire scr_write  = wr_ok & (reg_waddr == A_SCR);

    //  reads with side effects. reg_rd is only high for an accepted read, and these
    wire rbr_read   = reg_rd & (reg_raddr == A_DATA) & ~dlab;
    wire iir_read   = reg_rd & (reg_raddr == A_IIR);
    wire lsr_read   = reg_rd & (reg_raddr == A_LSR);
    wire msr_read   = reg_rd & (reg_raddr == A_MSR);

    // FIFO state
    wire rx_empty   = (rxf_count == 5'd0);
    wire tx_empty   = (txf_count == 5'd0);
    wire rx_full    = fen_q ? (rxf_count == 5'd16) : ~rx_empty;
    wire tx_full    = fen_q ? (txf_count == 5'd16) : ~tx_empty;
    wire cpu_pop    = rbr_read & ~rx_empty;
    wire rx_err     = rx_bi | rx_fe | rx_pe;
    wire head_err   = rxf_dout[10] | rxf_dout[9] | rxf_dout[8];

    // FIFO control writes
    wire fen_change = fcr_write & (reg_wdata[0] != fen_q);
    wire rx_reset   = fcr_write & (fen_change | (reg_wdata[0] & reg_wdata[1]));
    wire tx_reset   = fcr_write & (fen_change | (reg_wdata[0] & reg_wdata[2]));

    //  overrun: a character arrives, there is no room, and no read or reset makes room
    wire overrun    = rx_valid & rx_full & ~cpu_pop & ~rx_reset;

    // trigger level, FCR7:6
    wire [4:0] trig_level = (trig_q == 2'd0) ? 5'd1 :
                            (trig_q == 2'd1) ? 5'd4 :
                            (trig_q == 2'd2) ? 5'd8 : 5'd14;

    // interrupt sources and priority, Table 5
    wire src_rls = ier_q[2] & (oe_q | pe_q | fe_q | bi_q);
    wire src_rda = ier_q[0] & (fen_q ? (rxf_count >= trig_level) : ~rx_empty);
    wire src_cti = ier_q[0] & fen_q & to_pend_q;
    wire src_thr = ier_q[1] & thri_q;
    wire src_msi = ier_q[3] & (delta_q != 4'h0);

    wire [3:0] iir_id = src_rls ? ID_RLS  :
                        src_rda ? ID_RDA  :
                        src_cti ? ID_CTI  :
                        src_thr ? ID_THRI :
    ...
    wire thre_rise  = tx_empty & ~thre_prev_q;
    wire etbei_rise = ier_write & reg_wdata[1] & ~ier_q[1] & tx_empty;
    wire thri_set   = thre_rise | etbei_rise | fen_change;
    wire thri_clr   = thr_write | (iir_read & (iir_id == ID_THRI));

    //  modem status changes {DDCD, TERI, DDSR, DCTS}: any change, except RI which only
    wire [3:0] msr_edge = {msr_now[3] ^ msr_prev_q[3],
    ...
    wire [3:0] nbits      = 4'd6 + {2'b00, lcr_q[1:0]} + {3'b000, lcr_q[3]};
    wire [5:0] stop_ticks = !lcr_q[2]            ? 6'd16 :
                            (lcr_q[1:0] == 2'd0) ? 6'd24 : 6'd32;
    wire [7:0] char_ticks = {nbits, 4'b0000} + {2'b00, stop_ticks};   // at most 192
    wire [9:0] to_limit   = {char_ticks, 2'b00};                       // at most 768

    // register values as software sees them
    wire       lsr7_vis = fen_q & (lsr7_q | (errs_q != 5'd0));
    wire [7:0] lsr_val  = {lsr7_vis, tx_empty & ~tx_busy, tx_empty,
                           bi_q, fe_q, pe_q, oe_q, ~rx_empty};
    wire [7:0] msr_val  = {msr_now, delta_q};
    wire [7:0] iir_val  = {fen_q, fen_q, 2'b00, iir_id};
